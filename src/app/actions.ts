...
export async function getMembers(communityId: string): Promise<Member[]> {
  console.log(`[DEBUG] getMembers called for communityId: ${communityId}`);
  try {
    const client = await clientPromise;
    const db = client.db(dbName);
    console.log(`[DEBUG] Connected to MongoDB database: ${dbName}`);
    ...
    if (!community) {
      console.log(`[DEBUG] No community found with ID: ${communityId}`);
      return [];
    }
    
    console.log(`[DEBUG] Community found: ${community.name || 'unnamed'}`);
    
    if (!community.usersList) {
      console.log(`[DEBUG] Community has no usersList property`);
      return [];
    }
    
    if (!Array.isArray(community.usersList)) {
      console.log(`[DEBUG] Community usersList is not an array, type: ${typeof community.usersList}`);
      return [];
    }
    
    console.log(`[DEBUG] Community usersList has ${community.usersList.length} items`);
    
    const userIds = community.usersList.map((item: any) => {
      if (!item || !item.user) {
        console.log(`[DEBUG] Invalid usersList item:`, item);
        return null;
      }
      return item.user;
    }).filter(Boolean);
    
    console.log(`[DEBUG] Extracted ${userIds.length} valid user IDs from usersList`);
    
    if (userIds.length === 0) {
      console.log(`[DEBUG] No valid user IDs found in usersList`);
      return [];
    }
    
    console.log(`[DEBUG] Fetching users from database with ${userIds.length} IDs`);
    const users = await db.collection('users')
      .find({ _id: { $in: userIds } })
      .toArray();
    
    console.log(`[DEBUG] Found ${users.length} users out of ${userIds.length} IDs`);
    ...
    const members: Member[] = users.map(user => {
      ...
      const userListItem = community.usersList.find(
        (item: any) => item.user && item.user.toString() === user._id.toString()
      );
      
      if (!userListItem) {
        console.log(`[DEBUG] Could not find usersList item for user: ${user._id}`);
      }
      
      // Convert MongoDB role to Member role type
      let role: 'owner' | 'admin' | 'member' = 'member';
      if (userListItem?.role === 'admin' || userListItem?.role === 'commu_leader') {
        role = 'admin';
      } else if (community.owner && community.owner.toString() === user._id.toString()) {
        role = 'owner';
      }
      ...
      const member: Member = {
        ...
        uid: user._id.toString(), // Use MongoDB ID as UID for now
      ...
      console.log(`[DEBUG] Mapped member: ${member.displayName} (${member.id}) with role: ${member.role}`);
      return member;
    });
    
    console.log(`[DEBUG] Returning ${members.length} members`);
    return members;
  } catch (error) {
    console.error(`[ERROR] Error fetching members for community ${communityId}:`, error);
    return [];
  }
}

export async function getMessagesForMember(communityId: string, memberId: string): Promise<Message[]> {
  return getMessages(communityId, memberId);
}

export async function summarizeMessages({ communityId, userId, messages }: { communityId: string, userId: string, messages: { sender: string, text: string }[] }): Promise<{ summary: string }> {
  // Simple implementation that returns a summary based on the messages
  try {
    const messageCount = messages.length;
    const uniqueSenders = new Set(messages.map(m => m.sender)).size;
    
    // Create a simple summary
    const summary = `This conversation contains ${messageCount} messages from ${uniqueSenders} participants. The messages discuss various topics related to the community.`;
  ...
  try {
    ...
    const formattedMessages: Message[] = messages.map(msg => ({
      ...
      sender: {
        ...
        displayName: 'Unknown', // We would need to fetch user details separately
      },
      data: msg // Include the original message data
    }));
...
export interface CommunityExportDataResult {
  ...
  message: string;
  data?: any;
  exportData?: string; // Added for compatibility with existing code
}
